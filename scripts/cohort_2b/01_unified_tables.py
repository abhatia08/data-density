...
# MAGIC %md
# MAGIC **Purpose**: Person-date unified table for cohort_2b (cohort_1 spec)
...
clear_downstream_tables("cohort_2b", core_tables, use_widget=False, verbose=True, clear_downstream=CLEAR_DOWNSTREAM_TABLES)
...
def aggregate_temporal(df, temporal_col):
...
unified_weekly = aggregate_temporal(unified_temporal, "week")
unified_monthly = aggregate_temporal(unified_temporal, "month")
unified_yearly = aggregate_temporal(unified_temporal, "year")
...
unified_weekly = finalize(unified_weekly)
unified_monthly = finalize(unified_monthly)
unified_yearly = finalize(unified_yearly)
...
# MAGIC %sql
# MAGIC use catalog `hive_metastore`; select * from `cohort_2b`.`unified_daily`;

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `hive_metastore`; select * from `cohort_2b`.`unified_weekly` limit 100;

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `hive_metastore`; select * from `cohort_2b`.`unified_monthly` limit 100;

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `hive_metastore`; select * from `cohort_2b`.`unified_yearly` limit 100;
