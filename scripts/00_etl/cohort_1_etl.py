...
# MAGIC
# MAGIC **This program loads the Core OMDP Concept tables and the OMDP standardized patient data for Cohort 1 for the data density project.**
...
# MAGIC %md
# MAGIC ### 2. Create Cohort 1 Database  and load tables

# COMMAND ----------

# Define the 'cohort_1' database and point to the same ADLS Gen2 root path
cohort_db = "cohort_1"
base_path = "abfss://<container>@<storage-account>.dfs.core.windows.net/COHORT_1_OMOP_TABLES"

# Create the cohort_1 database if it doesn't already exist
...
# ETL loop - read each CSV into a DataFrame and write as a Delta table in cohort_1
for table in cohort_tables:
    ...
    # Write DataFrame to Delta table in cohort_1 (overwrite mode)
    df.write.format("delta") \
      .mode("overwrite") \
      .saveAsTable(f"{cohort_db}.{clean_name}")

    # Print confirmation
    print(f"Loaded cohort_1 table '{clean_name}' into database '{cohort_db}'.")
...
# MAGIC %md
# MAGIC ### 4. Cohort 1 Analysis - Patient Counts and Language Distribution
# MAGIC
...
print("=" * 60)
print("4. COHORT 1 - SUMMARY STATISTICS")
print("=" * 60)
