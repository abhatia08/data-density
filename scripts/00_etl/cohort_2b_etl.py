...
# MAGIC
# MAGIC **This program loads the Core OMDP Concept tables and the OMDP standardized patient data for Cohort 2B for the data density project.**
...
# MAGIC %md
# MAGIC ### 2. Create Cohort 2B Database  and load tables

# COMMAND ----------

# Define the 'cohort_2b' database and point to the same ADLS Gen2 root path
cohort_db = "cohort_2b"
base_path = "abfss://<container>@<storage-account>.dfs.core.windows.net/COHORT_2B_OMOP_TABLES"

# Create the cohort_2b database if it doesn't already exist
...
# ETL loop - read each CSV into a DataFrame and write as a Delta table in cohort_2b
for table in cohort_tables:
    ...
    # Write DataFrame to Delta table in cohort_2b (overwrite mode)
    df.write.format("delta") \
      .mode("overwrite") \
      .saveAsTable(f"{cohort_db}.{clean_name}")

    # Print confirmation
    print(f"Loaded cohort_2b table '{clean_name}' into database '{cohort_db}'.")
...
# MAGIC %md
# MAGIC ### 4. Cohort 2B Analysis - Patient Counts and Language Distribution
# MAGIC
...
print("=" * 60)
print("4. COHORT 2B - SUMMARY STATISTICS")
print("=" * 60)
