...
# MAGIC %md
# MAGIC **Purpose**: Build person-date unified table from OMOP domains
# COMMAND ----------
...
# MAGIC %md
# MAGIC ### 1.4 Clear Downstream Delta Tables
...
clear_downstream_tables("cohort_1", core_tables, use_widget=False, verbose=True, clear_downstream=CLEAR_DOWNSTREAM_TABLES)
...
# MAGIC %md
# MAGIC ### 5.1 Explode Visit Spans to Daily Records
...
# MAGIC %md
# MAGIC ### 5.2 Calculate Daily Visit Counts and Hospitalization Flags

# COMMAND ----------

# Count DISTINCT visits per person-date to avoid double-counting overlapping visits
...
# MAGIC %md
# MAGIC ### 6.1 Extract Concept-Only Dates

# COMMAND ----------

# Extract unique person-dates from all clinical concept domains
# This captures dates where concepts were recorded but no visit exists (e.g., standalone orders)
...
# MAGIC
# MAGIC Based on Charlson et al. (1987) with ICD code mappings from Quan et al. (2005)

# COMMAND ----------

# MAGIC %md
# MAGIC ### 7.1 Filter to Problem List Conditions
...
condition_start_filter = (
    ...
    .filter(F.col("condition_year") == F.col("visit_year"))  # Same year only
    .select(
...
# We use CASE WHEN logic to assign cc_group (1-17)
...
# Per Quan et al. 2005: certain codes appear in multiple comorbidity categories
# 40403, 40413, 40493: appear in both CHF (group 2) and renal disease (group 13)
# 4573: appears in both peripheral vascular (group 3) and cerebrovascular (group 4)
conditions_expanded = conditions.union(
    conditions.filter(
        ...
        "cc_group",
        F.when(F.substring(F.col("clean_code"), 1, 5).isin('40403','40413','40493'), 13)  # Also renal disease
        .when(F.substring(F.col("clean_code"), 1, 4) == '4573', 4)  # Also cerebrovascular
        .otherwise(0)
    )
)
...
# MAGIC %md
# MAGIC ### 7.5 Remove Duplicate Conditions per Visit
...
# Build wide-format daily table by joining visit counts and daily CCI to the person-date spine
...
# Left-join CCI data (based on prior visits)
unified_daily = unified_daily.join(
...
# MAGIC %md
# MAGIC ## 8. Add Summary Stats

# COMMAND ----------

# Use count_no_match from utils
...
unified_daily = (
    ...
    # FLAG: has any clinical concepts (composite flag across all domains)
    .withColumn(
...
# MAGIC %md
# MAGIC ## 9. Add Person-Level Data

# COMMAND ----------

# MAGIC %md
# MAGIC ### 9.1 Join Person Demographics and Calculate Birth Date
...
unified_daily = (
    ...
    .withColumn("birth_date",
        # Handle partial birth dates to avoid excluding patients with missing month/day
        F.when(
            F.col("pd_month").isNotNull() & F.col("pd_day").isNotNull(),
            F.to_date(F.concat_ws("-", "pd_year", "pd_month", "pd_day"))
        ).when(
            F.col("pd_month").isNotNull(),
            F.to_date(F.concat_ws("-", "pd_year", "pd_month", F.lit("01")))  # Default to 1st of month
        ).otherwise(
            F.to_date(F.concat_ws("-", "pd_year", F.lit("07"), F.lit("01")))  # Default to July 1 (mid-year)
        )
    )
...
# MAGIC %md
# MAGIC ### 9.2 Calculate Age at Day
...
unified_daily = (
    ...
    .withColumn(
        "gender",
        F.when(F.lower(F.col("clg.cl_gender_name")) == "female", "Female")
        .when(F.lower(F.col("clg.cl_gender_name")) == "male", "Male")
        .otherwise("Unknown")
    )
    .drop("pd_gender_cid", "clg.cl_gender_cid", "clg.cl_gender_name")
)

# COMMAND ----------

# MAGIC %md
# MAGIC ### 9.4 Map Ethnicity
...
unified_daily = (
    ...
    .withColumn(
        "ethnicity",
        F.when(F.lower(F.col("cle.cl_eth_name")).like("%not hispanic%"), "Not Hispanic or Latino")
        .when(F.lower(F.col("cle.cl_eth_name")).like("%hispanic%"), "Hispanic or Latino")
        .when(F.col("cle.cl_eth_name").isNull(), "Unknown")
        .otherwise("Not Hispanic or Latino")
    )
    .drop("pd_eth_cid", "cle.cl_eth_cid", "cle.cl_eth_name")
)
...
unified_daily = (
    ...
    .withColumn(
        "race",
        F.when(F.lower(F.col("clr.cl_race_name")).like("%asians%"), "Asian")
        .when(F.lower(F.col("clr.cl_race_name")).like("%white%"), "White")
        .when(F.lower(F.col("clr.cl_race_name")).like("%black%"), "Black")
        .when(F.lower(F.col("clr.cl_race_name")).like("%american indian%"), "American Indian")
        .when(F.lower(F.col("clr.cl_race_name")).like("%hawaiian%"), "Hawaiian")
        .otherwise("Unknown")
    )
    .drop("pd_race_cid", "clr.cl_race_cid", "clr.cl_race_name")
)

# COMMAND ----------

# MAGIC %md
# MAGIC ### 9.6 Finalize Column Order

# COMMAND ----------

# drop any column starting with those join prefixes
drop_prefixes = ["pd_", "clg_", "cle_", "clr_"]
...
concept_flag_cols = ["has_clinical_concepts"]  # Composite flag for any clinical concept
domain_array_cols = [
...
# MAGIC %md
# MAGIC ## 10. Aggregate by Week/Month/Year

# COMMAND ----------

# MAGIC %md
# MAGIC ### 10.1 Define Aggregation Function

# COMMAND ----------

# MAGIC %md
# MAGIC ### 10.2 Create Temporal Columns
...
# MAGIC %md
# MAGIC ### 10.3 Finalize Schema and Column Order
...
# MAGIC %md
# MAGIC ## 11. Save Tables

# COMMAND ----------

# MAGIC %md
# MAGIC ### 11.1 Create Output Database
...
# Unpersist cached arrays to free memory
...
# MAGIC %md
# MAGIC ## 13. Preview Tables

# COMMAND ----------

# MAGIC %md
# MAGIC ### 13.1 Preview Daily Table

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `hive_metastore`; select * from `cohort_1`.`unified_daily`;

# COMMAND ----------

# MAGIC %md
# MAGIC ### 13.2 Preview Weekly Table

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `hive_metastore`; select * from `cohort_1`.`unified_weekly` limit 100;

# COMMAND ----------

# MAGIC %md
# MAGIC ### 13.3 Preview Monthly Table
...
# MAGIC   *
# MAGIC from
# MAGIC   `cohort_1`.`unified_monthly`
# MAGIC limit 100;

# COMMAND ----------

# MAGIC %md
# MAGIC ### 13.4 Preview Yearly Table

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `hive_metastore`; select * from `cohort_1`.`unified_yearly` limit 100;